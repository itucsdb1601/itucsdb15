...
class academics:
	...
	def initialize_academics(config):
		with dbapi2.connect(config) as connection:
			...
			cursor.execute(query)
			query = """CREATE TABLE IF NOT EXISTS ACADEMICINFO (id serial primary key, uni_name VARCHAR(80) UNIQUE NOT NULL, level INTEGER, scholarship VARCHAR(3), mainresearch VARCHAR(100) NOT NULL, researchbudget INTEGER NOT NULL);"""
			cursor.execute(query)
			query = """INSERT INTO ACADEMICINFO (uni_name, level, scholarship, mainresearch, researchbudget) VALUES ('Istanbul Technical University', 3, 'y', 'Daylight Saving Time', 50000000), ('Harvard University', 1, 'y', 'Nobel Research', 10000000000), ('Mersin University',5, 'n', 'Agricultural Developments', 3000000);"""
			cursor.execute(query)
			query = """CREATE TABLE IF NOT EXISTS SCHOLARSHIPS (scholarship_id serial primary key, scholarshiptype VARCHAR(50) NOT NULL, scholarshipname VARCHAR (100) UNIQUE NOT NULL, scholars INTEGER, money_amount INTEGER NOT NULL);"""
			cursor.execute(query)
			...
			cursor.execute(query)
			query = """CREATE TABLE IF NOT EXISTS LEVELS (id serial perimary key, unilevel INTEGER, leveltype VARCHAR(80) UNIQUE NOT NULL);"""
			cursor.execute(query)
	...
	def academics_page(config):
		with dbapi2.connect(config) as connection:
			if request.method == 'GET':
				cursor = connection.cursor()
				query = "SELECT uni_name from ACADEMICINFO"
				cursor.execute(query)
