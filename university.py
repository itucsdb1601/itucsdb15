...
class university:
	...
	def initialize_universities(config):
		with dbapi2.connect(config) as connection:
			...
			cursor.execute(query)
			query = """CREATE TABLE IF NOT EXISTS UNICITY (city_id INTEGER primary key, city_name VARCHAR(80) NOT NULL);"""
			cursor.execute(query)
	...
	def universities_page(config):
		with dbapi2.connect(config) as connection:
			if request.method == 'GET':
				cursor = connection.cursor()
				query = "SELECT uni_name, unitype_id, unicity_id from UNIVERSITYLIST"
				cursor.execute(query)
	...
	def saveuniversity(config):
		...
		if request.method == 'POST':
			unitype_id = request.form['unitypeid']
	...
	def universities_page_delete(config, deleteuniversities):
		with dbapi2.connect(config) as connection:
			cursor = connection.cursor()
			query = "DELETE FROM UNIVERSITYLIST WHERE uni_id = '%d'"
	...
	def universities_page_update(config, updateuniversities):
		with dbapi2.connect(config) as connection:
			cursor = connection.cursor()
			query = """SELECT uni_name from UNIVERSITYLIST WHERE uni_name = '%s'"""
			cursor.execute(query)
			connection.commit();
			return render_template('universities_edit.html', unis = cursor) #cursora eşitlencek şeyi belirle
